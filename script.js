...
async function getRTCStats(statsObject){
  ...
  stats.forEach(report => {
    ...
    outputView += `<h2>Report: ${report.type}</h3>\n<strong>ID:</strong> ${report.id}<br>\n` +
    ...
    Object.keys(report).forEach(statName => {
      if (statName !== "id" && statName !== "timestamp" && statName !== "type") {
        outputView += `<strong>${statName}</strong>:${report[statName]}<br>\n`;
        output[statName] =  report[statName];
      }
    });

    document.querySelector(".stats-view").innerHTML = outputView;
    console.log(output);
